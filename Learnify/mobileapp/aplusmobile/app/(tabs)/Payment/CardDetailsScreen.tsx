  ...
  const handlePayment = async () => {
    ...
    try {
      const response = await fetch(
        `http://${LOCALHOST}:8080/api/payment/process-payment/${userId}`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        try {
          const premiumResponse = await fetch(
            `http://${LOCALHOST}:8080/api/updatePremiumStatus/12`,
            {
              method: "PATCH",
              headers: {
                "Content-Type": "application/json",
          ...
          if (premiumResponse.ok) {
            ...
            Alert.alert(
              "Error",
              premiumData.message || "Failed to upgrade your account."
            );
          }
        } catch (premiumError) {
          Alert.alert(
            "Error",
            "An error occurred while updating your account to premium."
          );
        ...
        switch (data.message) {
          ...
          case "Transaction failed":
            Alert.alert(
              "Error",
              `Transaction failed: ${data.error || "Please try again later."}`
            );
            break;
          default:
            Alert.alert(
              "Error",
              `An unexpected error occurred: ${data.message}`
            );
        }
      }
    } catch (error) {
      ...
      {cardType && (
        <View style={styles.detectedCardContainer}>
          <View style={styles.cardDetailsOverlay}>
            <Text style={styles.cardDetailsText}>{cardType}</Text>
          </View>
        </View>
      )}
