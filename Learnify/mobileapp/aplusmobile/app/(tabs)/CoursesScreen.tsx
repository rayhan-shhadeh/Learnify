...
const CoursesScreen = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        ...
        const response = await fetch(`http://192.168.68.58:8080/api/user/courses/${decoded?.id}`);
        if (!response.ok) {
  ...
  const addNewCourse = async () => {
    try {
      ...
      const decoded: { id: string } | null = jwtDecode<{ id: string }>(token);
      setUserId(decoded?.id ?? null); // Adjust this based on the token structure
      const response = await fetch(`http://192.168.68.58:8080/api/course`, {
        method: 'POST',
        headers: {
  ...
  const deleteCourse = async (courseId: string) => {
    try {
      ...
      const decoded: { id: string } | null = jwtDecode<{ id: string }>(token);
      setUserId(decoded?.id ?? null); // Adjust this based on the token structure
      const response = await fetch(`http://192.168.68.58:8080/api/course/${courseId}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
        },
